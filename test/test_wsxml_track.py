"""Tests for parsing tracks using MbXmlParser."""
import unittest
from musicbrainz2.wsxml import MbXmlParser, ParseError
from musicbrainz2.model import NS_MMD_1, NS_REL_1, Relation
import io
import os.path

VALID_DATA_DIR = os.path.join('test-data', 'valid')
INVALID_DATA_DIR = os.path.join('test-data', 'invalid')

VALID_TRACK_DIR = os.path.join(VALID_DATA_DIR, 'track')

def makeId(relativeUri, resType='track'):
	return 'http://musicbrainz.org/%s/%s' % (resType, relativeUri)


class ParseTrackTest(unittest.TestCase):

	def __init__(self, name):
		unittest.TestCase.__init__(self, name)


	def testTrackBasic(self):
		f = os.path.join(VALID_TRACK_DIR, 'Silent_All_These_Years_1.xml')
		md = MbXmlParser().parse(f)
		track = md.getTrack()

		self.assertFalse( track is None )
		self.assertEqual(track.getTitle(), 'Silent All These Years')
		self.assertEqual(track.getDuration(), 253466)


	def testTrackRelations(self):
		f = os.path.join(VALID_TRACK_DIR, 'Silent_All_These_Years_2.xml')
		md = MbXmlParser().parse(f)
		track = md.getTrack()

		self.assertFalse( track is None )
		self.assertEqual(track.getTitle(), 'Silent All These Years')
		self.assertEqual(track.getDuration(), 253466)

		trackRels = track.getRelations(Relation.TO_TRACK)
		self.assertEqual(len(trackRels), 1)
		rel1 = trackRels[0]
		self.assertEqual(rel1.getType(), NS_REL_1 + 'Cover')
		self.assertEqual(rel1.getDirection(), Relation.DIR_BACKWARD)
		self.assertEqual(rel1.getTargetId(),
			makeId('31e1c0c4-967f-435e-b09a-35ee079ee234', 'track'))
		self.assertTrue( rel1.getBeginDate() is None )
		self.assertTrue( rel1.getEndDate() is None )

		self.assertFalse( rel1.getTarget() is None )
		self.assertEqual(rel1.getTarget().getId(),
			makeId('31e1c0c4-967f-435e-b09a-35ee079ee234'))
		self.assertEqual(rel1.getTarget().getArtist().getId(),
			makeId('5bcd4eaa-fae7-465f-9f03-d005b959ed02', 'artist'))


	def testTrackFull(self):
		f = os.path.join(VALID_TRACK_DIR, 'Silent_All_These_Years_4.xml')
		md = MbXmlParser().parse(f)
		track = md.getTrack()

		self.assertFalse( track is None )
		self.assertEqual(track.getTitle(), 'Silent All These Years')
		self.assertEqual(track.getDuration(), 253466)

		artist = track.getArtist()
		self.assertFalse( artist is None )
		self.assertEqual(artist.getId(),
			makeId('c0b2500e-0cef-4130-869d-732b23ed9df5', 'artist'))
		self.assertEqual(artist.getType(), NS_MMD_1 + 'Person')
		self.assertEqual(artist.getName(), 'Tori Amos')
		self.assertEqual(artist.getSortName(), 'Amos, Tori')

		puids = track.getPuids()
		self.assertEqual(len(puids), 7)
		self.assertEqual(puids[0], 'c2a2cee5-a8ca-4f89-a092-c3e1e65ab7e6')
		self.assertEqual(puids[6], '42ab76ea-5d42-4259-85d7-e7f2c69e4485')
		
		isrcs = track.isrcs
		self.assertEqual(len(isrcs), 1)
		self.assertEqual(isrcs[0], 'USPR37300012')

		releases = track.getReleases()
		self.assertEqual(len(releases), 1)
		self.assertEqual(releases[0].getTitle(), 'Little Earthquakes')
		self.assertEqual(releases[0].getTracksOffset(), 2)

	def testSearchResults(self):
		f = os.path.join(VALID_TRACK_DIR, 'search_result_1.xml')
		md = MbXmlParser().parse(f)

		self.assertEqual(md.trackResultsOffset, 7)
		self.assertEqual(md.trackResultsCount, 100)

		results = md.getTrackResults()
		self.assertEqual(len(results), 3)

		self.assertEqual(results[0].getScore(), 100)
		track1 = results[0].getTrack()
		self.assertEqual(track1.getTitle(), 'Little Earthquakes')

# EOF
